...
**The file you want is `render/solved.json`.** It is committed, so you do not
have to run anything to get it — clone and read.

| file | what it is |
|---|---|
| `render/solved.json` | the corrected board: 9 parts, 51 checks passing, nothing to flag |
| `render/naive.json` | the same BOM placed badly: 7 failures, each with an overlay to draw in red |

Both are regenerated by `./run_demo.sh` and are byte-stable, so they will not
churn under you. `out/` is scratch and is gitignored — do not read from there.
