...
def post(handler,token):
 ...
 try:
  ...
  if handler.path=='/assembly/upload':result=pipeline.upload(unquote(handler.headers.get('X-Filename','')),data,handler.headers.get('X-Units','mm'),handler.headers.get('X-Role','product'))
  else:
   payload=json.loads(data)
   if handler.path=='/assembly/confirm':result=pipeline.confirm(payload['revision'])
